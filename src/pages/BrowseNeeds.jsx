  ...
  const mockNeeds = [
    {
      ...
      description: "My mother needs immediate surgery for a heart condition. The medical bills are beyond our family's means.",
    ...
    },
    {
      ...
      description: "Single mother struggling to pay school fees for three children. Education is their path to a better future.",
    ...
    },
    {
      ...
      description: "Facing eviction due to job loss. Need help with rent payment to keep my family housed.",
    ...
    },
    {
      ...
      description: "Our community kitchen serves 200+ families daily. Need funds to continue operations this month.",
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Find verified requests to support</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              Showing {sortedNeeds.length} of {mockNeeds.length} verified needs
            </p>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 rounded-full bg-primary"></div>
              <span className="text-sm text-muted-foreground">All requests are verified</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {sortedNeeds.map((need) => (
            <Card key={need.id} className="border-0 shadow-soft hover:shadow-elegant transition-all duration-300">
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground leading-tight">{need.title}</h3>
                      <p className="text-sm text-muted-foreground">by {need.recipient}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end space-y-1">
                    {need.urgent && <Badge variant="destructive" className="text-xs">Urgent</Badge>}
                    {need.verified && <Badge variant="default" className="text-xs bg-primary/10 text-primary">Verified</Badge>}
                  </div>
                </div>
              </CardHeader>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {getProgress(need.raised, need.amount)}% of ${need.amount.toLocaleString()}
                    </span>
                  </div>
                  <Progress value={getProgress(need.raised, need.amount)} className="h-2" />
                </div>
